pfpaChartSVG = function(m) {
  ...
  const W=760,H=520,pad={l:64,r:12,t:32,b:38};
  ...
  for(let val=min;val<=max;val+=step) {
    const gy=y(val);
    grids+=`<line class="chart-grid" x1="${pad.l}" y1="${gy}" x2="${W-pad.r}" y2="${gy}"></line><text class="chart-axis-text chart-y-label" x="${pad.l-9}" y="${gy+5}" text-anchor="end">${fmt.format(val)}</text>`;
  }
  ...
  const labels=points.map((s,i)=>{
    ...
    return show ? `<text class="chart-axis-text" x="${x(i)}" y="${H-11}" text-anchor="middle">${s.year}</text>` : '';
  }).join('');
