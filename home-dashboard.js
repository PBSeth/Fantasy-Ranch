...
function leagueHighlights() {
  ...
  // Auction-era Top Pick investment records (2021+, plus current 2026 headliners).
  const auctionPicks=[];
  managers.forEach(m=>{
    (m.seasons||[]).forEach(s=>{
  ...
  auctionPicks.forEach(x=>{
    ...
    row.total+=x.price; row.count++; spendByManager.set(x.m.id,row);
  });
